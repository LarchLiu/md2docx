...
function generateThemesData() {
  ...
  const outFile = path.join(__dirname, 'src/host/themes-data.ts');

  const dirs = ['presets', 'code-themes', 'color-schemes', 'layout-schemes', 'table-styles'];
  const exports = [];
  ...
  for (const dir of dirs) {
    ...
    for (const file of files) {
      ...
      const varName = `${dir.replace(/-/g, '_')}_${id.replace(/-/g, '_')}`;
      exports.push(`export const ${varName} = ${content.trim()} as const;`);
    }
  }

  // Also export registry.json
  const registryPath = path.join(themesDir, 'registry.json');
  if (fs.existsSync(registryPath)) {
    exports.push(`export const registry = ${fs.readFileSync(registryPath, 'utf8').trim()} as const;`);
  }

  // Export font-config.json
  const fontConfigPath = path.join(themesDir, 'font-config.json');
  if (fs.existsSync(fontConfigPath)) {
    exports.push(`export const fontConfig = ${fs.readFileSync(fontConfigPath, 'utf8').trim()} as const;`);
  }
...
`;
  ...
  console.log('Generated themes-data.ts');
}

function generateTemplatesData() {
  const templatesDir = path.join(__dirname, 'src/templates');
  const outFile = path.join(__dirname, 'src/host/templates-data.ts');
 ...
 * Generated by build.mjs from node/src/templates/
 */
...
`;
  ...
  fs.writeFileSync(outFile, code, 'utf8');
  console.log('Generated templates-data.ts');
}
...
async function build() {
  ...
  try {
    ...
    // Build core.ts for Node.js workers (ESM format)
    console.log('Building core (Node.js worker)...');
    const coreNodeCtx = await esbuild.context({
      entryPoints: {
        'core-worker': path.join(__dirname, 'src/host/core.ts'),
      },
      bundle: true,
      platform: 'node',
      target: 'node18',
    ...
    // Build core.ts for browser workers (ESM format)
    console.log('Building core (browser worker ESM)...');
    const coreBrowserCtx = await esbuild.context({
      entryPoints: {
        'core-worker-browser': path.join(__dirname, 'src/host/core.ts'),
      },
      bundle: true,
      outdir: rendererOutDir,
      format: 'esm',
...
<body>
  ...
  <script src="./puppeteer-runtime-core.js"></script>
    ...
    if (watch) {
      await mainCtx.watch();
      await coreNodeCtx.watch();
      await rendererCtx.watch();
      await coreBrowserCtx.watch();
    ...
    console.log('Tip: run `node node/dist/md2x.js --help` in another terminal.');
      ...
      const shutdown = async () => {
        ...
        try {
          await coreNodeCtx.dispose();
        } catch {}
        try {
          await rendererCtx.dispose();
        } catch {}
        try {
          await coreBrowserCtx.dispose();
        } catch {}
        process.exit(0);
      };
